...
CREATE TABLE IF NOT EXISTS Vehicle (
	bastidor CHAR(10) PRIMARY KEY,
    matricula CHAR(7),
    dataVenda DATE,
    color VARCHAR(20),
    preuCost DECIMAL(7,2),
    preuVenda DECIMAL(7,2)
) ENGINE = INNODB;
...
SELECT bastidor, preuCost FROM Vehicle ORDER BY preuCost DESC;
...
SELECT MIN(preuCost) AS "Cost Mínim", MAX(preuCost) AS "Cost Maxim" FROM Vehicle;

/* 3b. Obtener los vehiculos con coste más caro y más barato.*/


/* 4. Obtenir el total del preu de cost de tots els vehicles.*/
SELECT SUM(preuCost) AS "Preu Total" FROM Vehicle;
...
SELECT * FROM Vehicle ORDER BY preuCost, preuVenda DESC;

/* 7. Obtenir l'import obtingut de la venda de tots els vehicles.*/
SELECT SUM(PreuVenda) AS "Precio Venta Total "FROM Vehicle;
...
SELECT bastidor, CONCAT(FORMAT(preuCost-preuVenda,"c")," €") AS "Beneficio" FROM Vehicle ORDER BY preuCost-preuVenda DESC;

/* 10.Obtenir totes els anys que s’han venut vehicles. El títol de la columna ha de ser “Anys amb ventes”.*/
SELECT DISTINCT YEAR(dataVenda) AS "Años con ventas" FROM Vehicle WHERE matricula IS NOT NULL;

/* 11.Obtenir tota la informació de tots els cotxes de color groc disponibles per a la venda.*/
SELECT * FROM Vehicle WHERE color = "Blanc" AND matricula IS NOT NULL;
...
SELECT bastidor, datediff(now(),dataVenda) AS 'DiasCirculando' FROM Vehicle
	WHERE matricula IS NOT NULL
	ORDER BY DiasCirculando DESC
	LIMIT 5;
